...
class MinimalPublisher(Node):
    ...
    def compute_joint_trajectory(self):
        ...
        # TODO find the joint with the largest distance to target
        # TODO

        # TODO find the time needed by the joint with max dist
        total_time = #TODO 
        ticks = 10
        ...
        for i, (q0, qf) in enumerate(zip(initial_position, target_position)):
            ...
            # TODO here use the lspb function (check the file trap_trajectory.py)
            ts, q, qd, ok = #TODO

            # TODO check that the trajectory is achievable, if not adjust the number of ticks and the time
            #      so that this joint arrives earlier at target
            if ok== ...#TODO
